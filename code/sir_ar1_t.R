...
for(ii in 2:d)
{
  theta[ii] = rt(1, 6, 0.2*theta[ii-1])
}
x1 <- rt(d, 4, theta)
...
# f(theta[1:n]|x[1:n])                (f(x[i]|theta[i])*f(theta[i]|theta[i-1])))
# ---------------------------- ~  prod(----------------------------------------)
# g(theta[1:n])                       (g(theta[i]|theta[i-1])                  )

#        f(x[i]|theta[i])*f(theta[i]|theta[i-1]))
# w[i] = ----------------------------------------
#        g(theta[i]|theta[i-1])
...
{
  d = length(theta)
  aux1 = dt(x[d], 4, theta[d])
  aux2 = ifelse(d == 1,
                dt(theta[d], 6, 0),
                dt(theta[d], 6, phi*theta[d-1])
  )              
  aux3 = ifelse(d == 1,
                dnorm(theta[d], 0, sigma_t),
                dnorm(theta[d], phi*theta[d-1], sigma_t)
  )
  aux1*aux2/aux3
}
