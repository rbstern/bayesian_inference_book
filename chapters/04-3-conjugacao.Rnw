...
$\mu|x   \sim N(177,0.05)$. A Figura \ref{fig:normal_normal} mostra a distribuição a priori e a distribuição a posteriori. Note que a posteriori tem a maior parte de sua massa para valores de $\mu$ mais próximos do dado observado. 
 ...
 <<normal_normal,cache=TRUE, fig.pos="t", fig.height=3, fig.width=4, fig.cap="Distribuições a priori e a posteriori (depois de observamos um indivíduo com 1,8m) para a altura média de um brasileiro",fig.path="../",fig.align="center">>=
library(ggplot2)
...
$(\mu,\tau^2)|x   \sim \mbox{Normal-Gamma}(1.725,2,6.5,0.055625)$. A Figura \ref{fig:normal_gamma} mostra a distribuição a priori e a distribuição a posteriori para essa observação. 
 ...
 <<normal_gamma,cache=TRUE, fig.pos="t", fig.height=3, fig.width=4, fig.cap="Distribuições a priori e a posteriori (depois de observamos um indivíduo com 1,8m) para a altura média de um brasileiro",fig.path="../",fig.align="center">>=
library(ggplot2)
